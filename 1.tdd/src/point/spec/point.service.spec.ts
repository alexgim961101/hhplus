...
describe('PointService', () => {
    ...
    describe('chargeUserPoint', () => {
        test('특정 유저의 포인트 충전 기능', async () => {
            ...
            jest.spyOn(mockUserPoint, 'selectById').mockResolvedValue({
                ...expectedUserPoint,
                point: 0,
            });
        ...
        test('DB 업데이트 실패시 예외가 발생한다', async () => {
            ...
            // when & then
            await expect(service.chargeUserPoint(userId, amount)).rejects.toThrow('DB Error');
        });
    });
    ...
    describe('useUserPoint', () => {
        ...
        test('DB 업데이트 실패시 예외가 발생한다', async () => {
            ...
            // when & then
            await expect(service.useUserPoint(userId, amount)).rejects.toThrow('DB Error');
        });
    });
});
