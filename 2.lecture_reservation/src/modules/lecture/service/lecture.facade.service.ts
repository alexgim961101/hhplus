...
export class LectureFacadeService {
    constructor(private readonly lectureService: LectureService, private readonly reservationService: ReservationService) {}
    ...
    async getAvailableLectures(userId: number, dateTime: Date) {
        ...
        const filteredLectures = availableLectures.filter(lecture => !reservations.some(reservation => reservation.lectureId === lecture.id));
